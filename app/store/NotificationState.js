...
import socket from "../libaxios/socket";
...
const useNotificationStore = create((set) => ({
  ...
  fetchNotificationById: async (id) => {
    ...
    try {
      ...
      set({ error: error.response?.data?.message || "Failed to fetch notification", loading: false });
  ...
  createNotification: async (data) => {
    ...
    try {
      ...
      set((state) => ({
        notifications: [...state.notifications, { ...data, id: response.data.id }],
        loading: false,
      }));
  ...
  MarkAsRead: async (id) => {
    try {
      const response = await axiosB.put(`/api/notifications/${id}`);
      set((state) => ({
