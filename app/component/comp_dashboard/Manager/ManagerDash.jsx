...
const ManagerDashboard = () => {
  const { managers, fetchAllManagers, createManager, updateManager, deleteManager, updateManagerFeaturedStatus } = useManagerStore();
        ...
        <TableBody>
          {managers.map((manager) => (
            <TableRow key={manager.id}>
              ...
              <TableCell>
                {manager.image && (
                  <img src={manager.image} alt={manager.name} className="w-10 h-10 rounded-full" />
                )}
              </TableCell>
  ...
  onCheckedChange={(checked) => handleToggleFeatured(manager.id, checked ? 1 : 0)} // Convert boolean back to 1 or 0
/>

              </TableCell>
              <TableCell>
                <Button onClick={() => handleEdit(manager)}>Edit</Button>
                <Button variant="destructive" onClick={() => handleDeleteClick(manager.id)}>
                  Delete
                </Button>
              </TableCell>
            </TableRow>
        ...
        <DialogContent className="max-h-[80vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit}>
            <div className="space-y-4">
              ...
              <Input type="number" name="academy_size" value={formData.academy_size} onChange={handleInputChange} />

              <Label>Scouted Players</Label>
              <Input type="number" name="scouted_players" value={formData.scouted_players} onChange={handleInputChange} />

              <Label>Pro Player Promotions</Label>
              <Input type="number" name="pro_player_promotions" value={formData.pro_player_promotions} onChange={handleInputChange} />

              <Label>Certifications</Label>
              <Input name="certifications" value={formData.certifications} onChange={handleInputChange} />

              <Label>Focus Areas</Label>
              <Input name="focus_areas" value={formData.focus_areas} onChange={handleInputChange} />

              <Label>Digital Scouting Participation</Label>
              <Input type="number" name="digital_scouting_participation" value={formData.digital_scouting_participation} onChange={handleInputChange} />
            </div>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Are you sure you want to delete this manager?</DialogTitle>
          </DialogHeader>
          <DialogFooter>
