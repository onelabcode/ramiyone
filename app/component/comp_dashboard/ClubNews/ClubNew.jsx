...
export default function ClubNewsDashboard() {
  const { clubNews, fetchAllClubNews, createClubNews, updateClubNews, deleteClubNews, loading } = useClubNewsStore();
      ...
      <Card>
        ...
        <CardContent>
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{editingNews ? "Edit Club News" : "Create Club News"}</DialogTitle>
                <DialogDescription>
                  {editingNews ? "Update the club news details." : "Fill in the details to create a new club news."}
                </DialogDescription>
              </DialogHeader>
              <form onSubmit={editingNews ? handleUpdateNews : handleCreateNews} className="space-y-4">
                ...
                <Input type="file" onChange={(e) => setImage(e.target.files[0])} required={!editingNews} />
                <DialogFooter>
            ...
            <TableBody>
              {clubNews.map((item) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell>
                    <Button variant="ghost" onClick={() => { setEditingNews(item); setTitle(item.title); setLink(item.link); setIsDialogOpen(true); }}>Edit</Button>
                    <Button variant="ghost" onClick={() => { setNewsToDelete(item.id); setIsDeleteDialogOpen(true); }}>Delete</Button>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This action cannot be undone. This will permanently delete the club news.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
