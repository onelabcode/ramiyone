  ...
  return (
    <section className="w-1/3 flex flex-col space-y-5">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="space-y-2">
          <label className="block text-sm font-semibold">YouTube Video URL</label>
          <input
            type="url"
            ...
            className="text-sm pl-5 border-2 border-gray-200 rounded-full p-4 w-full"
          />
          {errors.video && <p className="text-red-500 text-xs">{errors.video.message}</p>}
        </div>
        <button type="submit" className="bg-blue-400 text-white py-3 rounded-full font-semibold text-base px-4">
