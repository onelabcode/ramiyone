    ...
    for (const player of players) {
      if (player.created_by && !coachNameMap[player.created_by]) {
        try {
          ...
          console.error(`Error fetching coach for ID ${player.created_by}:`, error);
    ...
    <>
      <div className="rounded-md border">
        <Table>
          ...
          <TableBody>
            {players.map((player) => (
              <TableRow key={player.id} className="cursor-pointer hover:bg-muted/50" onClick={() => setSelectedPlayer(player)}>
                <TableCell className="font-medium">
                  <div className="flex items-center gap-3">
                    <Avatar className="h-10 w-10">
                      <AvatarImage src={player.image} alt={player.player_id} />
                      <AvatarFallback>{player.player_id.slice(0, 2)}</AvatarFallback>
                    </Avatar>
                    <div>
                ...
                <TableCell>{coachNames[player.created_by] || "Loading..."}</TableCell>
                <TableCell>
