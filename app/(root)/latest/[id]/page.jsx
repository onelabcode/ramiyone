...
export default function Home() {
  ...
  const { getTutorById,getFeaturedTutors,currentTutor,featuredTutors}=useTutorStore();
     const {id}=useParams();
     useEffect(() => {
        getTutorById(id);
        getFeaturedTutors();
        ...
        <article className="bg-white rounded-2xl shadow-xl overflow-hidden">
          ...
          <div className="p-8">
            <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
              ...
              <time dateTime={currentTutor.created_at}>{format(new Date(currentTutor.created_at), 'MMM d, h:mm a')}</time>
            </div>
            <h1 className="text-3xl font-bold text-gray-900 mb-6">{currentTutor.title}</h1>
            <div
