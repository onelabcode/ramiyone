...
export default function BlogPostEach() {
 const {id}=useParams();
const {singleblog, fetchBlogById,fetchRecommendedBlogs,recommendedBlogs}=useBlogStore();
useEffect(() => {
    fetchBlogById(id);
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className=" px-4 text-center">
            ...
            <div className="flex items-center justify-center gap-2 text-white">
              ...
              <span className="text-lg">{format(new Date(singleblog.created_at), 'MMM d, h:mm a')}</span>
            </div>
          </div>
        </div>
      </div>
