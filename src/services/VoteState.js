...
const useTopPlayersStore = create((set) => ({
  ...
  fetchTopPlayers: async () => {
    ...
    try {
      ...
      set({
        error: error.response?.data || "Error fetching top players",
        loading: false,
      });
    }
  },
  ...
  updateTopPlayers: async (players) => {
    ...
    try {
      ...
      set({
        error: error.response?.data || "Error updating top players",
        loading: false,
      });
    }
  },
  ...
  clearTopPlayers: async () => {
    ...
    try {
      ...
      set({
        error: error.response?.data || "Error clearing top players",
        loading: false,
      });
    }
  },
  voteForPlayer: async (userId, playerId) => {
    ...
    try {
      ...
      set({
        error: error.response?.data.message || "Error voting for the player",
        loading: false,
      });
      toast.error(
