...
const useFindPlayerStore = create((set) => ({
  ...
  createPlayerRequest: async (data) => {
    ...
    try {
      ...
      set({
        error: error.response?.data?.message || "Failed to create player request",
        loading: false,
      });
  ...
  fetchPlayerRequests: async () => {
    ...
    try {
      ...
      set({
        error: error.response?.data?.message || "Failed to fetch player requests",
        loading: false,
      });
  ...
  deletePlayerRequest: async (id) => {
    ...
    try {
      ...
      set((state) => ({
        playerRequests: state.playerRequests.filter((request) => request.id !== id),
        loading: false,
      }));
      ...
      set({
        error: error.response?.data?.message || "Failed to delete player request",
        loading: false,
      });
  ...
  updatePlayerRequestStatus: async (id, status) => {
    ...
    try {
      ...
      set({
        error: error.response?.data?.message || "Failed to update player request status",
        loading: false,
      });
